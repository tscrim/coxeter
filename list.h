...
public:
  ...
  void operator delete(void *ptr) { return arena().free(ptr, sizeof(List<T>)); }
...
template <class T> inline T &List<T>::operator[](Ulong j) { return d_ptr[j]; }
template <class T> inline void List<T>::setData(const T *source, Ulong r) {
  setData(source, 0, r);
}
template <class T> void List<T>::setSizeValue(const Ulong &n) { d_size = n; }
