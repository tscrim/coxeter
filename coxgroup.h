...
public:
  ...
  virtual bool inOrder(List<Length> &a, const CoxWord &h,
                       const CoxWord &g) const;                 /* inlined */
...
inline kl::KLContext &CoxGroup::kl() {
  activateKL();
  return *d_kl;
}
inline invkl::KLContext &CoxGroup::invkl() {
  activateIKL();
  return *d_invkl;
}
...
inline String &CoxGroup::append(String &str, const Generator &s) const {
  return appendSymbol(str, s, interface());
}
...
inline void CoxGroup::printSymbol(FILE *file, const Generator &s) const {
...
template <class H> inline void CoxGroup::printHeckeElt(FILE *file, const H &h) {
