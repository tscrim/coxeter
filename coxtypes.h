...
const ParSize LPARNBR_MAX =
    COXNBR_MAX - RANK_MAX - 1;                      /* top value is reserved */
...
public:
  /* constructors */
  void operator delete(void *ptr) { return arena().free(ptr, sizeof(CoxWord)); }
  ...
  CoxWord &setSubWord(const CoxWord &h, const Length &first, const Length &r);
};
